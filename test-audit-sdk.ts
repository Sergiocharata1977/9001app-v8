...
async function testAuditSDK() {
  ...
  try {
    ...
    if (audits.length > 0) {
      ...
      console.log(
        `      - Norm Points: ${sampleAudit.selectedNormPoints.length}`
      );
      console.log('');
    }
    ...
    if (createdAudit) {
      ...
      console.log(
        `      - Norm Points: ${createdAudit.selectedNormPoints.join(', ')}\n`
      );
    }
    ...
    console.log(
      '🎉 Your Audit SDK (Sepeck) is properly configured and working.\n'
    );
    console.log('📊 Summary:');
    ...
    console.error(
      '   3. Ensure the service account has read/write permissions'
    );
