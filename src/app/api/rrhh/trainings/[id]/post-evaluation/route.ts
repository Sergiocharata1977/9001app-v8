...
export async function POST(
   ...
   try {
    ...
    console.error('Error en POST /api/rrhh/trainings/[id]/post-evaluation:', error);
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('no encontrada') || error.message.includes('no tiene participantes')) {
        return NextResponse.json(
          { error: error.message },
          { status: 404 }
        );
