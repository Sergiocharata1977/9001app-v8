...
export async function PUT(
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Error al actualizar personal', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    console.error(`Error in DELETE /api/sdk/rrhh/personnel/${params.id}:`, error);
    return NextResponse.json(
      { error: 'Error al eliminar personal', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
