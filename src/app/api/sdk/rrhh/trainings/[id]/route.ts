...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Error al obtener entrenamiento', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
...
export async function PUT(
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Error al actualizar entrenamiento', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    console.error(`Error in DELETE /api/sdk/rrhh/trainings/${params.id}:`, error);
    return NextResponse.json(
      { error: 'Error al eliminar entrenamiento', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
