...
export async function GET() {
  try {
    ...
    if (recordsCount > 0) {
      ...
      const listsSnapshot = await getDocs(collection(db, 'processRecords', firstRecord.id, 'kanbanLists'));
      kanbanListsCount = listsSnapshot.size;
      
      // Contar tarjetas en todas las listas
      for (const listDoc of listsSnapshot.docs) {
        const cardsSnapshot = await getDocs(collection(db, 'processRecords', firstRecord.id, 'kanbanLists', listDoc.id, 'cards'));
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        overallStatus: definitionsCount > 0 && recordsCount > 0 ? 'ready' : 'needs_seeding'
      }
    });
  } catch (error) {
