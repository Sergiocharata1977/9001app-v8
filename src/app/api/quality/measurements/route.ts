...
import { measurementSchema, measurementFiltersSchema } from '@/lib/validations/quality';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const filters = measurementFiltersSchema.parse({
      ...
      validation_status: searchParams.get('validation_status') as any || undefined,
    ...
    if (filters.indicator_id) {
      measurements = await MeasurementService.getByIndicator(filters.indicator_id);
    } else if (filters.objective_id) {
      measurements = await MeasurementService.getByObjective(filters.objective_id);
    } else if (filters.process_definition_id) {
      measurements = await MeasurementService.getByProcess(filters.process_definition_id);
    } else {
    ...
    if (filters.search) {
      measurements = measurements.filter(m =>
        ...
        m.measurement_method.toLowerCase().includes(filters.search!.toLowerCase())
      );
    }

    if (filters.validation_status) {
      measurements = measurements.filter(m => m.validation_status === filters.validation_status);
    }

    if (filters.measured_by) {
      measurements = measurements.filter(m => m.measured_by === filters.measured_by);
    }

    if (filters.start_date) {
      measurements = measurements.filter(m => m.measurement_date >= filters.start_date!);
    }

    if (filters.end_date) {
      measurements = measurements.filter(m => m.measurement_date <= filters.end_date!);
    }
