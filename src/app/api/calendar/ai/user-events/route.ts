...
/**
 * POST /api/calendar/ai/user-events
 * Obtener eventos de un usuario con filtros avanzados para IA
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const { userId, filters, includeContext, sortBy, sortOrder } = body;
    ...
    const organizationId =
      filters && 'organizationId' in filters
        ? ((filters as Record<string, unknown>).organizationId as
            | string
            | undefined)
        : undefined;
