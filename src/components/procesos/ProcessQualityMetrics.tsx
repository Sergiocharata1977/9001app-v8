...
export function ProcessQualityMetrics({
  ...
  const activeObjectives = objectives.filter(obj => obj.status === 'activo').length;
  ...
  const complianceRate = totalObjectives > 0 ? Math.round((completedObjectives / totalObjectives) * 100) : 0;

  const totalIndicators = indicators.length;
  const activeIndicators = indicators.filter(ind => ind.status === 'activo').length;
  ...
  const avgProgress = totalObjectives > 0
    ? Math.round(objectives.reduce((sum, obj) => sum + obj.progress_percentage, 0) / totalObjectives)
    : 0;
  ...
  const metrics = [
    {
      ...
      bgColor: complianceRate >= 80 ? 'bg-green-100' : complianceRate >= 60 ? 'bg-yellow-100' : 'bg-red-100',
    },
    {
    ...
    },
    {
      ...
      icon: TrendingUp,
      color: avgProgress >= 70 ? 'text-green-600' : avgProgress >= 50 ? 'text-yellow-600' : 'text-red-600',
      bgColor: avgProgress >= 70 ? 'bg-green-100' : avgProgress >= 50 ? 'bg-yellow-100' : 'bg-red-100',
    },
    {
  ...
  const getStatusSummary = () => {
    ...
    return Object.entries(statusCounts).map(([status, count]) => ({
      status,
      count,
      percentage: totalObjectives > 0 ? Math.round((count / totalObjectives) * 100) : 0
    }));
  };
                ...
                <div key={status} className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    <Badge className={
                      ...
                      status === 'activo' ? 'bg-blue-100 text-blue-800' :
                    ...
                    <span className="text-sm text-gray-600">{count} objetivos</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <div className="w-24 bg-gray-200 rounded-full h-2">
                      <div
                        className={`h-2 rounded-full ${
                          ...
                          status === 'activo' ? 'bg-blue-600' :
                          status === 'atrasado' ? 'bg-red-600' :
                          'bg-gray-600'
                        }`}
          ...
          <div className="space-y-3">
            {complianceRate >= 80 && (
              <div className="flex items-center space-x-2 text-green-700">
                ...
                <span className="text-sm">Excelente cumplimiento de objetivos</span>
              </div>
            )}
            ...
            {objectivesAtRisk > 0 && (
              <div className="flex items-center space-x-2 text-red-700">
                ...
                <span className="text-sm">{objectivesAtRisk} objetivos requieren atención inmediata</span>
              </div>
            )}
            ...
            {recentMeasurements === 0 && totalIndicators > 0 && (
              <div className="flex items-center space-x-2 text-yellow-700">
                ...
                <span className="text-sm">No hay mediciones recientes en los indicadores</span>
              </div>
            )}
            ...
            {totalObjectives === 0 && (
              <div className="flex items-center space-x-2 text-blue-700">
                ...
                <span className="text-sm">Define objetivos SMART para este proceso</span>
              </div>
            )}
          </div>
        </CardContent>
