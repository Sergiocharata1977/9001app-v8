...
export function FileUploader({
  ...
  const processFiles = useCallback(
    async (files: FileList | File[]) => {
      ...
      for (const upload of newUploads.filter(u => u.status === 'uploading')) {
        ...
        onFileUploaded?.(upload.file, `uploaded-${upload.id}`);
      }
    },
    [uploads.length, maxFiles, onFilesSelected, onFileUploaded]
  );
  ...
  const removeFile = (fileId: string) => {
    setUploads(prev => {
      ...
      const removed = prev.find(u => u.id === fileId);
      if (removed?.preview) {
