...
export const CreateCalendarEventSchema = z.object({
  ...
  eventType: z.enum(['audit', 'meeting', 'training', 'deadline', 'review', 'other']),
  startDate: z.date().or(z.string().datetime()),
  endDate: z.date().or(z.string().datetime()),
  location: z.string().max(200).optional(),
  attendees: z.array(z.string().uuid()).min(1, 'Debe haber al menos un asistente'),
...
export const UpdateCalendarEventSchema = z.object({
  ...
  eventType: z.enum(['audit', 'meeting', 'training', 'deadline', 'review', 'other']).optional(),
